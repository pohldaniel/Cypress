...
exports.closeExtraTargets = exports.open = exports.closeProtocolConnection = exports.connectProtocolToBrowser = exports.connectToExisting = exports.connectToNewSpec = exports.clearInstanceState = exports.CDPFailedToStartFirefox = void 0;
...
class CDPFailedToStartFirefox extends Error {
    constructor(message) {
        super(message);
        this.name = CDPFailedToStartFirefox.ERROR_NAME;
    }
    static isCDPFailedToStartFirefoxError(error) {
        return (error === null || error === void 0 ? void 0 : error.name) === CDPFailedToStartFirefox.ERROR_NAME;
    }
}
exports.CDPFailedToStartFirefox = CDPFailedToStartFirefox;
CDPFailedToStartFirefox.ERROR_NAME = 'CDPFailedToStartFirefox';
const debug = (0, debug_1.default)('cypress:server:browsers:firefox');
const debugVerbose = (0, debug_1.default)('cypress-verbose:server:browsers:firefox');
...
function shouldUseBiDi(browser) {
    try {
...
async function connectToNewSpec(browser, options, automation) {
    if (shouldUseBiDi(browser)) {
        ...
        await firefox_util_1.default.connectToNewSpecBiDi(options, automation, browserBidiClient);
        debug('registering middleware');
        automation.use(browserBidiClient.automationMiddleware);
    }
    else {
        debug('connectToNewSpec cdp');
        await firefox_util_1.default.connectToNewSpecCDP(options, automation, browserCriClient);
    }
}
...
function closeProtocolConnection() {
    throw new Error('Protocol is not yet supported in firefox.');
}
exports.closeProtocolConnection = closeProtocolConnection;
...
async function open(browser, url, options, automation) {
    ...
    else if (os_1.default.platform() === 'win32' || os_1.default.platform() === 'darwin') {
    ...
    const [marionettePort, webDriverBiDiPort,] = await Promise.all([(0, get_port_1.default)(), (0, get_port_1.default)()]);
    ...
    if (!options.isTextTerminal) {
        const doesPathExist = await fs_extra_1.default.pathExists(profileDir);
        if (doesPathExist) {
    ...
    const geckoDriverOptions = {
        ...
        jsdebugger: debug_1.default.enabled(GECKODRIVER_DEBUG_NAMESPACE_VERBOSE) || false,
        log: debug_1.default.enabled(GECKODRIVER_DEBUG_NAMESPACE_VERBOSE) ? 'debug' : 'error',
        logNoTruncate: debug_1.default.enabled(GECKODRIVER_DEBUG_NAMESPACE_VERBOSE),
    };
    ...
    // @ts-expect-error
    let browserInstanceWrapper = new events_1.default();
    ...
    try {
         ...
         */
        const base64EncodedProfile = await new Promise((resolve, reject) => {
            profile.encoded(function (err, encodedProfile) {
                err ? reject(err) : resolve(encodedProfile);
            });
        });
        const newSessionCapabilities = {
            logLevel: debug_1.default.enabled(WEBDRIVER_DEBUG_NAMESPACE_VERBOSE) ? 'info' : 'silent',
            capabilities: {
                alwaysMatch: {
                    browserName: 'firefox',
        ...
        const WD = webdriver_1.WebDriver.getWebDriverPackage();
        ...
        const browserPID = webdriverClient.capabilities['moz:processID'];
        debug(`firefox running on pid: ${browserPID}`);
        const driverPID = webdriverClient.capabilities['wdio:driverPID'];
        ...
        await Promise.all(lodash_1.default.map(launchOptions.extensions, async (path) => {
        ...
        const client = await firefox_util_1.default.setup({ automation, url, webdriverClient, remotePort: cdpPort, useWebDriverBiDi: USE_WEBDRIVER_BIDI, onError: options.onError });
        if (client instanceof browser_cri_client_1.BrowserCriClient) {
