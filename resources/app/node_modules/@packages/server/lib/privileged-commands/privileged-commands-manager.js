...
exports.privilegedCommandsManager = void 0;
...
class PrivilegedCommandsManager {
    ...
    async getPrivilegedChannel(options) {
        ...
        if (this.channelKeys[options.url])
            return;
        const key = (0, uuid_1.v4)();
        this.channelKeys[options.url] = key;
        const script = (await fs_1.fs.readFileAsync(path_1.default.join(__dirname, 'privileged-channel.js'))).toString();
    ...
    hasVerifiedCommand(command) {
        const matchingCommand = lodash_1.default.find(this.verifiedCommands, ({ name, args }) => {
            return command.name === name && lodash_1.default.isEqual(args, command.args);
        });
    ...
    runPrivilegedCommand(config, { commandName, options, args }) {
        ...
        if (config.testingType === 'e2e' && !hasCommand) {
            ...
            const err = new Error(`cy.${commandName}() must be invoked from the spec file or support file`);
            err.isNonSpec = true;
            throw err;
        }
        switch (commandName) {
            case 'exec':
                return exec_1.default.run(config.projectRoot, options);
            case 'origin':
                // only need to verify that it's spec-originated above
                return;
            case 'readFile':
                return files_1.default.readFile(config.projectRoot, options);
            case 'selectFile':
                return files_1.default.readFiles(config.projectRoot, options);
            case 'writeFile':
                return files_1.default.writeFile(config.projectRoot, options);
            case 'task': {
                ...
                return task_1.default.run(configFile !== null && configFile !== void 0 ? configFile : null, options);
            }
            default:
...
exports.privilegedCommandsManager = new PrivilegedCommandsManager();
