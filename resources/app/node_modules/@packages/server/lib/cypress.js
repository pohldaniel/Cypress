...
const args_1 = tslib_1.__importDefault(require("./util/args"));
const telemetry_1 = require("../../telemetry");
const data_context_1 = require("../../data-context");
const errors_1 = require("./errors");
const debug = (0, debug_1.default)('cypress:server:cypress');
const exit = async (code = 0) => {
    // TODO: we shouldn't have to do this
    // but cannot figure out how null is
    // being passed into exit
    debug('about to exit with code', code);
    if ((0, data_context_1.hasCtx)()) {
        await (0, data_context_1.getCtx)().lifecycleManager.mainProcessWillDisconnect().catch((err) => {
            debug('mainProcessWillDisconnect errored with: ', err);
        });
    }
    const span = telemetry_1.telemetry.getSpan('cypress');
    span === null || span === void 0 ? void 0 : span.setAttribute('exitCode', code);
    span === null || span === void 0 ? void 0 : span.end();
    await telemetry_1.telemetry.shutdown().catch((err) => {
        debug('telemetry shutdown errored with: ', err);
    });
    return process.exit(code);
};
const showWarningForInvalidConfig = (options) => {
    const publicConfigKeys = (0, config_1.getPublicConfigKeys)();
    ...
    if (invalidConfigOptions.length && options.invokedFromCli) {
        return (0, errors_1.warning)('INVALID_CONFIG_OPTION', invalidConfigOptions);
    }
    return undefined;
};
const exit0 = () => {
...
module.exports = {
        ...
        return bluebird_1.default.try(() => {
            ...
            if (this.isCurrentlyRunningElectron()) {
                ...
                if (!options.invokedFromCli) {
                    (0, errors_1.warning)('INVOKED_BINARY_OUTSIDE_NPM_MODULE');
                }
    ...
    start(argv = []) {
        ...
        let options;
        try {
            options = args_1.default.toObject(argv);
        ...
        (_a = telemetry_1.telemetry.exporter()) === null || _a === void 0 ? void 0 : _a.attachRecordKey(options.key);
        if (options.headless) {
