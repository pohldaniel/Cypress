...
const artifact_1 = require("../cloud/artifacts/artifact");
...
const printSkippedArtifact = (label, message = 'Nothing to upload', error) => {
...
exports.printSkippedArtifact = printSkippedArtifact;
const logUploadManifest = (artifacts, protocolCaptureMeta, protocolFatalError) => {
    const labels = {
        [artifact_1.ArtifactKinds.VIDEO]: 'Video',
        [artifact_1.ArtifactKinds.SCREENSHOTS]: 'Screenshot',
        [artifact_1.ArtifactKinds.PROTOCOL]: 'Test Replay',
    };
    // eslint-disable-next-line no-console
    console.log('');
    terminal_1.default.header('Uploading Cloud Artifacts', {
        color: ['blue'],
    });
    // eslint-disable-next-line no-console
    console.log('');
    const video = artifacts.find(({ reportKey }) => reportKey === artifact_1.ArtifactKinds.VIDEO);
    const screenshots = artifacts.filter(({ reportKey }) => reportKey === artifact_1.ArtifactKinds.SCREENSHOTS);
    const protocol = artifacts.find(({ reportKey }) => reportKey === artifact_1.ArtifactKinds.PROTOCOL);
    if (video) {
        (0, exports.printPendingArtifactUpload)(video, labels);
    }
    else {
        (0, exports.printSkippedArtifact)('Video');
    }
    if (screenshots.length) {
        screenshots.forEach(((screenshot) => {
            (0, exports.printPendingArtifactUpload)(screenshot, labels);
        }));
    }
    else {
        (0, exports.printSkippedArtifact)('Screenshot');
    }
    // if protocolFatalError exists here, there is not a protocol artifact to attempt to upload
    if (protocolFatalError) {
        (0, exports.printSkippedArtifact)('Test Replay', 'Failed Capturing', protocolFatalError.error.message);
    }
    else if (protocol) {
        if (!protocolFatalError) {
            (0, exports.printPendingArtifactUpload)(protocol, labels);
        }
    }
    else if (protocolCaptureMeta.disabledMessage) {
        (0, exports.printSkippedArtifact)('Test Replay', 'Nothing to upload', protocolCaptureMeta.disabledMessage);
    }
};
exports.logUploadManifest = logUploadManifest;
const printCompletedArtifactUpload = ({ pathToFile, key, fileSize, success, error, uploadDuration }, labels, num) => {
...
const logUploadResults = (results, protocolFatalError) => {
    if (!results.length) {
        return;
    }
    const labels = {
    ...
    // eslint-disable-next-line no-console
    console.log('');
    terminal_1.default.header('Uploaded Cloud Artifacts', {
        color: ['blue'],
    });
    // eslint-disable-next-line no-console
    console.log('');
    results.forEach(({ key, ...report }, i, { length }) => {
        (0, exports.printCompletedArtifactUpload)({ key, ...report }, labels, chalk_1.default.grey(`${i + 1}/${length}`));
    });
};
exports.logUploadResults = logUploadResults;
